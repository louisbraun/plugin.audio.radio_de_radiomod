...
def show_my_stations():
    ...
    return __add_stations(stations, add_custom=False, browse_more=None, add_more_stations=True)

@plugin.route('/show_more_stations')
def show_more_stations():
    items = (
        ...
        {'label': _('add_custom'),'fanart': __get_plugin_fanart(), 'icon': plugin.icon,
...
def get_stream_url(station_id):
    ...
    if station:
        ...
        __log('set last played station to %s' % station_id)
...
def __add_stations(stations, add_custom=False, browse_more=None, add_more_stations=False):
    items = []
    ...
    if add_more_stations:
        items.append({
            'label': 'Sender suchen', 'icon': plugin.icon,
            'fanart': __get_plugin_fanart(),
            'path': plugin.url_for('show_more_stations'),
        })
